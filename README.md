...
python -m pip install git+https://github.com/Main-Roads/merge-segments.git@v1.2.0
...
result = merge.on_slk_intervals(
    ...
    column_actions=[
        merge.Action(
            ...
            rename="column1_longest",
        ),
        merge.Action(
            "column1", merge.Aggregation.LengthWeightedAverage(), "column1_avg"
        ),
...
segmentation = segmentation.dropna(
    subset=[CN.road_number, CN.carriageway, CN.slk_from, CN.slk_to]
)
...
# Rename columns to our standard names:
pavement_data = pavement_data.rename(
    columns={
...
# Drop rows where critical fields are blank
pavement_data = pavement_data.dropna(
    subset=[CN.road_number, CN.carriageway, CN.slk_from, CN.slk_to]
)
...
segmentation_pavement = merge.on_slk_intervals(
    ...
    column_actions=[
        merge.Action(
            CN.pavement_total_width, merge.Aggregation.LengthWeightedAverage()
        ),
...
Latest release: **1.2.0** (2026-08-06).
