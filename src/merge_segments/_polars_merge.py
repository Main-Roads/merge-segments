...
_ROW_IDX_COL = "__merge_segments_target_row_idx__"
_DATA_IDX_COL = "__merge_segments_data_row_idx__"
_SEGMENT_LEN_COL = "__merge_segments_segment_len__"
...
def _ensure_polars_dataframe(name: str, frame: object) -> pl.DataFrame:
    if pl is None:
        raise ImportError(
            ...
            "Install with: pip install merge_segments[polars]"
        )
...
def on_slk_intervals_polars(
    ...
    if verbose:
        print(
            f"[merge_segments] Polars merge: {len(column_actions)} action(s), "
    ...
    if verbose:
        print(f"[merge_segments] Polars merge completed in {elapsed:.2f}s")
